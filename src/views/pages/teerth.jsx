...
import { PackageCard } from '@/views/components/teerth'
import { Link } from '@/lib/router'
import TrustBlock from '@/views/components/ui/trust-block'
import '@/styles/pages.css'

export default function Teerth() {
  const { divineDarshan, packages, teerthBenefits, teerthHero } = useTeerthCatalog()
  ...
  useEffect(() => {
    const id = setInterval(() => {
      setActive((cur) => {
        const next = (cur + 1) % teerthHero.length
        goTo(next)
        return next
      })
    }, 5200)
