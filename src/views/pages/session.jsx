...
import { PhoneIcon } from '@/views/components/ui/icons'
...
export default function Session({ mode, id }) {
  const { chatReplies, getAstrologer } = useAstrologyCatalog()
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      setSpent(a.price)
      if (mode === 'chat') {
        setMsgs([{ from: 'them', text: chatReplies[0], at: Date.now() }])
        replyIx.current = 1
      }
    }, 1400)
  ...
  const send = () => {
    ...
    setDraft('')
    setTimeout(() => {
      const reply = chatReplies[replyIx.current % chatReplies.length]
      replyIx.current += 1
