...
export function ProfileList() {
  const { genders, relations } = useProfileData()
...
export function Vouchers() {
  const { voucherCats, vouchers } = useProfileData()
...
export function PaymentMethods() {
  const { paymentKinds } = useProfileData()
  ...
  return (
    <div className="wrap flow-page">
      ...
      {payments.length === 0 ? (
        ...
        <ul className="acc-list">
          {payments.map((m) => {
            const kind = paymentKinds.find((k) => k.id === m.kind)
            return (
              <li key={m.id}>
      ...
      {edit && (
        <FilterSheet
          title={edit.id ? 'Edit payment method' : `Add ${paymentKinds.find((k) => k.id === edit.kind)?.label}`}
          onClose={() => setEdit(null)}
          cta="Save Changes"
        ...
        >
          <div className="bf-fields">
            <label><span>{paymentKinds.find((k) => k.id === edit.kind)?.label} *</span>
              <input
                value={edit.label}
                placeholder={paymentKinds.find((k) => k.id === edit.kind)?.hint}
...
export function Addresses() {
  const { addressTags } = useProfileData()
          ...
          <div className="bf-fields">
            <label><span>Save as</span>
              <select value={edit.label} onChange={(e) => setEdit({ ...edit, label: e.target.value })}>
                {addressTags.map((t) => <option key={t}>{t}</option>)}
              </select></label>
            <label><span>Full name *</span>
              <input value={edit.name} onChange={(e) => setEdit({ ...edit, name: e.target.value })} placeholder="Enter name" /></label>
...
export function Support() {
  const { issueSubjects, resolutions, supportTopics } = useProfileData()
  ...
  const topics = supportTopics.filter((t) => !q.trim() || (t.t + t.s).toLowerCase().includes(q.toLowerCase()))
          ...
          <ul className="acc-list">
            {tickets.map((t) => (
              <li key={t.id}>
                ...
                <div>
                  <strong>{t.id}</strong>
                  <small>{issueSubjects.find((s) => s.id === t.subject)?.label || t.subject} · {resolutions.find((r) => r.id === t.resolution)?.label}</small>
                  <small>{t.when}</small>
                </div>
