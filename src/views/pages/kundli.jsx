...
export function KundliList() {
  const { houses, planets } = useAstroDashboard()
...
export function KundliCreate() {
  const { birthPlaces, genders, kundliSteps } = useAstroDashboard()
  ...
  const next = () => {
    ...
    if (step < kundliSteps.length - 1) { setStep(step + 1); return }
    const k = addKundli({
            ...
            {step === 4 && (
              <div className="bf-fields">
                <label><span>Birth place *</span>
                  ...
                  />
                  <datalist id="places">
                    {birthPlaces.map((p) => <option key={p} value={p} />)}
                  </datalist>
                </label>
                <div className="k-suggest">
                  {birthPlaces.slice(0, 5).map((p) => (
        ...
        <aside className="flow-side">
          ...
          <div className="flow-nav">
            ...
            <button className="cta-wide" onClick={next} disabled={!valid}>
              {step === kundliSteps.length - 1 ? 'Generate Kundli' : 'Continue'}
