...
import { PujaCard } from '@/views/components/puja'
import { PinIcon, StarIcon } from '@/views/components/ui/icons'
import '@/styles/pages.css'

export default function PujaDetail({ slug }) {
  const { benefits, faqs, getPuja, getTemple, isSpecialPuja, loading, pujaModes, pujas, reviews } = usePujaCatalog()
  const puja = getPuja(slug)
  const special = puja ? isSpecialPuja(puja) : false

  const [picked, setPicked] = useState(null)
  const mode = picked || pujaModes.find((m) => m.popular) || pujaModes[0]
              ...
              <div className="dt-packages">
                {pujaModes.map((m) => (
                  <div className={`dt-package${m.popular ? ' pop' : ''}`} key={m.id}>
                    ...
                    <button onClick={() => { setPicked(m); book() }}>Choose {m.name}</button>
                  </div>
                ))}
              </div>
            </>
