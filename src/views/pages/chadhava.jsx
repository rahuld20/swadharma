...
export default function Chadhava() {
  const { chadhavaTypes, chadhavas, combos, getTemple, temples } = useChadhavaCatalog()
  const [temple, setTemple] = useState('all')
  const [sort, setSort] = useState('popular')
  /* Derived for the same reason as the puja tabs. */
  const [pickedKind, setPickedKind] = useState(null)
  const kind = pickedKind || (chadhavaTypes.some((t) => t.id === query().get('type'))
    ? query().get('type')
    : 'all')
