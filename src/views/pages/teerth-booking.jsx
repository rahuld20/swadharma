...
import { useTeerthCatalog } from '@/controllers/use-teerth'
...
export function TeerthBooking({ slug }) {
  const { accommodations, getPackage, insurance, loading } = useTeerthCatalog()
  ...
  const [pickedAcc, setPickedAcc] = useState(null)
  const acc = pickedAcc || accommodations.find((a) => a.popular) || accommodations[0]
          ...
          <section className="flow-card">
            ...
            <div className="pd-modes">
              {accommodations.map((a) => (
                <button key={a.id} className={`pd-mode${acc.id === a.id ? ' sel' : ''}`} onClick={() => setPickedAcc(a)}>
...
export function TeerthConfirmation() {
  const { insurance, prepSteps } = useTeerthCatalog()
