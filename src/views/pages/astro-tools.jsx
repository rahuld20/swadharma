...
import { useAstroDashboard } from '@/controllers/use-astrology'
...
export function Horoscope() {
  const { horoscope, horoscopeDays, zodiac } = useAstroDashboard()
  const [picked, setPicked] = useState(null)
  const [day, setDay] = useState('today')
  const sign = picked || zodiac[4]   // Leo, as in the bundled sample
  /* After every hook, so the count never changes between renders. Only ever
     true while a live API is still answering. */
  if (!sign) return null
  const h = horoscope
...
export function Panchang() {
  const { panchangCities, panchangData } = useAstroDashboard()
  const p = panchangData
