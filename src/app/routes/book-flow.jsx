...
export default function BookFlow({ kind, slug }) {
  ...
  const steps = FLOW_STEPS[kind][special ? 'special' : 'normal']

  const [stepIx, setStepIx] = useState(0)
  const [mode, setMode] = useState(() => {
    const q = query().get('mode')
    return PUJA_MODES.find((m) => m.id === q) || PUJA_MODES.find((m) => m.popular)
  })
        ...
        <ol className="flow-steps">
          {steps.map((s, i) => (
            <li key={s} className={i === stepIx ? 'now' : i < stepIx ? 'past' : ''}>
              <span>{i < stepIx ? '✓' : i + 1}</span>{STEP_LABEL[s]}
            </li>
          ))}
        </ol>
                    ...
                    <div className="pd-modes">
                      {PUJA_MODES.map((m) => (
                        <button
                          ...
                          onClick={() => { setMode(m); syncDevotees(m.people) }}
                        >
