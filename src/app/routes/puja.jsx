...
import {
  CHADHAVAS, CHALLENGES, PUJAS, PUJA_BANNERS, PUJA_HERO, PUJA_TRUST,
  PUJA_TYPES, TEMPLES,
} from '@/features/puja/api'
import { ChadhavaMini, PujaCard } from '@/features/puja/components'
...
export default function Puja() {
  const [type, setType] = useState(() => {
    ...
    return PUJA_TYPES.some((t) => t.id === q) ? q : 'all'
  })
  ...
  return (
    <div className="module-page">
      ...
      <section className="pj-hero" style={{ backgroundImage: `url(${PUJA_HERO.image})` }}>
        <div className="wrap pj-hero-in">
          <h1>{PUJA_HERO.title}</h1>
          <p className="pj-hero-date">{PUJA_HERO.date}</p>
      ...
      <section className="wrap">
        <div className="pj-banners">
          <PromoSlider
            ...
            perView={2}
            slides={PUJA_BANNERS.map((b) => ({ ...b, href: `puja/${b.slug}` }))}
          />
        </div>
