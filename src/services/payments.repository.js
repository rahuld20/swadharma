 ...
 */
import { BRAND, IS_MOCK, PAYMENTS_ENABLED, RAZORPAY_KEY_ID } from '@/config/app'
...
export async function payWithRazorpay({ amount, receipt, name, customer = {} }) {
  ...
  // ---- real gateway ----
  await loadSdk()
  const order = await http.post(endpoints.paymentOrder, { amount: paise, currency: 'INR', receipt })
  ...
  await http.post(endpoints.paymentVerify, {
