...
- **Footer** — the same 47 items desktop shows, and a check enforces that
  number rather than trusting it. Only the arrangement changes: the brand
  block and the four link groups stack instead of sitting side by side, and
  each group lays its links out in two columns so a section reads as three
  short rows rather than six long ones. Order down the page is brand,
  contact, social, the four groups, the app buttons, then the legal line.

  It used to collapse to the legal line alone — `.ftr-top` and `.ftr-apps`
  were `display: none` below 860px — which put the support address, all 24
  links and the app buttons out of reach on a phone entirely.
